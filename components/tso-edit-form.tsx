...
export function TsoEditForm({ initialData, users }: TsoEditFormProps) {
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await fetch(`/api/admin/tso-users/${initialData.id}`, {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ password, status, assignedToUserId }),
            });

            if (!response.ok) {
                throw new Error("Something went wrong while updating the account.");
            }
            ...
            <div className="space-y-4">
                ...
                <div>
                    ...
                    <select
                        ...
                        className="text-slate-900 w-full p-3 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-slate-900 focus:border-transparent outline-none transition-all"
                    >
                ...
                <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                        Selecting a student will automatically mark the account as Assigned.
                    </p>
                </div>
            </div>
